...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['mortal']) && !isset($_POST['search'])) {
    ...
    $query_insert = "INSERT INTO usuarios (nombre, cancion, enlace, apreciacion) VALUES ($1, $2, $3, $4)";
    $params = array($mortal, $cancion, $enlace, $apreciacion);
    $insert_result = pg_query_params($conn, $query_insert, $params);

    if ($insert_result) {
        header("Location: index.php");
        exit();
    } else {
        echo "Error al insertar el usuario: " . pg_last_error($conn);
    }
}
...
if (isset($_POST['search'])) {
    $search = $_POST['search'];
    $query = "SELECT * FROM usuarios WHERE nombre ILIKE $1 OR cancion ILIKE $1 ORDER BY id DESC";
                ...
                <?php if ($resultado && pg_num_rows($resultado) > 0): ?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><a href="actualizar.php?id=<?php echo $row['id']; ?>" class="btn-edit">Actualizar</a></td>
                            <td>
